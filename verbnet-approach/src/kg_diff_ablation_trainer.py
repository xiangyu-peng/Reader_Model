...
class Reader_Model_Trainer_KG_diff(object):
    ...
    def __init__(self, args, prompt_text=None):
        ...
        self.rocnli = RocNLI(args.rocnli_path, self.args.device)
        ...
        self.goal_all_set = defaultdict(list)
        ...
        # save story
        self.args.save_path += '_comet' if args.comet_use else ''
        self.args.save_path += '_conceptnet' if args.conceptnet_use else ''
        self.args.save_path += '_' + args.bart_or_roberta
        self.args.save_path += '_graph_coeff_' + str(args.graph_coeff).replace('.', '-') + '_' + str(args.graph_coeff_rel).replace('.', '-')
        self.args.save_path += '.txt'
        ...
        #graph distance
        self.graph_coeff = args.graph_coeff
        self.graph_coeff_rel = args.graph_coeff_rel
    ...
    def graph_distance(self, obj=None, sentence=None, idx=0):
        ...
        inference_nodes = set()
        for rel in self.goal_all_set:
            for nodes in self.goal_all_set[rel]:
                inference_nodes.add(nodes[0])
                inference_nodes.add(nodes[1])
        distance_inf = len(old_nodes.intersection(inference_nodes)) / (len(inference_nodes) + 1)  # larger is better

        # print('>>>', obj, sentence, distance, distance_inf, distance_rel/self.goal_rel_len)

        return distance + self.graph_coeff * distance_inf + self.graph_coeff_rel * distance_rel/self.goal_rel_len, distance
    ...
    def top_k_prunning(self, actions_dicts, goal_word=None, distance_type="vebnet"):
        ...
        if self.remain_div:
            ...
            id = 0
            print('distance_lst', distance_lst, distance_lst[id][1][0])
            while len(res) < self.topk:
                if distance_lst and distance_lst[id][1][0] not in res_set:
                    res.append(distance_lst[id])
                    res_set.add(distance_lst[id][1][0])
                if id < len(distance_lst) - 1:  # 可能会出现conceptnet extend的不够的情况
                    id += 1
            return res
    ...
    def action_extract(self, sentences_generated, standard="verb", idx=0):
        ...
        if standard == "verb":
            ...
            """
            print('sentences_generated', sentences_generated)
            best_one, prob_best = "", 100000
    ...
    def roberta_action_obj_gen(self, GPT_gen_sig=False, idx=0):
        ...
        for i in range(self.topk):  # generate topk GPT-2 sentences
            sentence = self.TE_MC_GPT_gen(self.prompt[idx])[0]
            print('GPT generate =>', sentence)
            obj_entity_candidates[str(i)] = [[sentence]]

        # if GPT_gen_sig:
        #     return sentence, obj_entity_candidates
        ...
        if self.standard == "verb":
            ...
            # print("verb => best_one, obj_dict", best_one, obj_dict)
    ...
    def bart_action_obj_gen(self, GPT_gen_sig=False, idx=0):
        """
        Generate actions from BART un-fine-tuned.
        :return:
        """
        sentences_generated = []
        obj_entity_candidates = defaultdict(list)
        ...
        for obj_entity in obj_entity_list:
            sent_lst = []
            # 1st consider roles '....., jenny and linda <mask> obj
            sentence_one = self.verb_generator.next_verb(prompt=self.prompt[idx].split(".")[-2] + ". " + " and ".join(self.roles),
                                                                                             obj=obj_entity,
                                                                                             return_sentence=True,
                                                                                             top_number=1)[0]
            # 1st consider roles '....., jenny and linda ... obj <mask>
            print('single role', self.prompt[idx].split(".")[-2] + ". " + " and ".join(self.roles), '---', obj_entity, '===>', sentence_one)
            sentence_one = " and ".join(self.roles) + ' ' + sentence_one
            sentence_one += ' ' + self.verb_generator.next_verb(prompt=self.prompt[idx].split(".")[-2] + ". " + sentence_one,
                                          return_sentence=True,
                                          top_number=1)[0]
            print('2. sentence_one', sentence_one)
            sent_lst.append(sentence_one)

            if len(self.roles) > 1:
                for role in self.roles:  # consider roles '....., jenny <mask> obj
                    sentence_one = role + ' ' + self.verb_generator.next_verb(
                        prompt=self.prompt[idx].split(".")[-2] + ". " + role,
                        obj=obj_entity,
                        return_sentence=True,
                        top_number=1)[0]
                    print('single role', self.prompt[idx].split(".")[-2] + ". " + role, '---', sentense_one)
                    # 1st consider roles '....., jenny ... obj <mask>
                    sentence_one += ' ' + self.verb_generator.next_verb(prompt=self.prompt[idx].split(".")[-2] + ". " + sentence_one,
                                                                 return_sentence=True,
                                                                 top_number=1)[0]
                    sent_lst.append(sentence_one)
            ...
            # consider roles '....., <mask> obj
            sentence_one = self.verb_generator.next_verb(
                prompt=self.prompt[idx].split(".")[-2] + ".",
                obj=obj_entity,
                return_sentence=True,
                top_number=1)[0]
            sentence_one += ' ' + self.verb_generator.next_verb(prompt=self.prompt[idx].split(".")[-2] + ". " + sentence_one,
            ...
            # sent_gen = self.roberta.get_prediction(sent_lst)  # no period
            sentences_generated += sent_lst
            obj_entity_candidates[obj_entity].append(sent_lst)
    ...
    def prepare_next_step(
        ...
        if goal_or_build == "build":
                    ...
                    for i in range(len(outputs[rel])):
                        ...
                        for node in node_1:
                            ...
                            for entity in node_2:
                                ...
                                all_set['entities'] += entity
                                for _rel in cs_dict:
                                    inference_set[_rel] += cs_dict[_rel]
                                    all_set[_rel] += cs_dict[_rel]
                    outputs[rel] = res
            ...
            self.goal_all_set = copy.deepcopy(all_set)
    ...
    def generate_actions(self):
        ...
        for i in range(len(self.node_history)):
            ...
            if self.args.comet_use:
                ...
                new_inference = set()
            print('!!!new_inference', new_inference)
            ...
            for entity in self.node_history[i]:
                if entity not in self.node_history_set[i] and entity not in [
                    ...
                    if self.args.conceptnet_use:
                        ...
                        print("cs_dict", cs_dict, self.roles, entity)
                    else:
                        cs_dict = []
            ...
            if self.args.bart_or_roberta == 'roberta':  # use roberta
                ...
                new_prompt, action_dict = self.bart_action_obj_gen(GPT_gen_sig, idx=i)
                action_dict_lst.append(action_dict)
                cs_entity_KG_dict_lst.append(cs_entity_KG_dict)
